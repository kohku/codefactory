    ...
    public MembershipUserCollection GetMembers(int maximumRows, int startRowIndex)
    {
        ...
        if (!string.IsNullOrEmpty(_rolenameToMatch))
        {
            ...
            foreach (string usr in Roles.GetUsersInRole(_rolenameToMatch))
                if (!string.IsNullOrEmpty(usr) && Membership.FindUsersByName(usr).Count > 0)
                    users.Add(Membership.GetUser(usr));

            totalRecords = users.Count;
        }
        else if (!string.IsNullOrEmpty(_usernameToMatch))
            users = Membership.FindUsersByName(_usernameToMatch, startRowIndex, maximumRows, out totalRecords);
        else if (!string.IsNullOrEmpty(_emailToMatch))
            users = Membership.FindUsersByEmail(_emailToMatch, startRowIndex, maximumRows, out totalRecords);
        else
            users = Membership.GetAllUsers(startRowIndex, maximumRows, out totalRecords);
