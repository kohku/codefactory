...
namespace CodeFactory.ContentManager.WebControls.WebParts
{
    internal static class SecUtility
    {
            ...
            bool checkForCommas, int maxSize, string paramName)
        {
            ...
            if (param.Length < 1)
                throw new ArgumentException(ResourceStringLoader.GetResourceString(
                    "Parameter_array_empty", new object[] { paramName }), paramName);
            ...
            for (int i = param.Length - 1; i >= 0; i--)
            {
                ...
                if (hashtable.Contains(param[i]))
                    throw new ArgumentException(ResourceStringLoader.GetResourceString(
                        "Parameter_duplicate_array_element", new object[] { paramName }), paramName);

                hashtable.Add(param[i], param[i]);
            }
        }

        internal static void CheckParameter(ref string param, bool checkForNull, bool checkIfEmpty, 
            bool checkForCommas, int maxSize, string paramName)
        {
            if (param == null)
            {
            ...
            else
            {
                ...
                if (checkIfEmpty && (param.Length < 1))
                    throw new ArgumentException(ResourceStringLoader.GetResourceString(
                        "Parameter_can_not_be_empty", new object[] { paramName }), paramName);

                if ((maxSize > 0) && (param.Length > maxSize))
                    throw new ArgumentException(ResourceStringLoader.GetResourceString(
                        "Parameter_too_long", new object[] {
                            paramName,
                ...
                if (checkForCommas && param.Contains(","))
                    throw new ArgumentException(ResourceStringLoader.GetResourceString(
                        "Parameter_can_not_contain_comma", new object[] { paramName }), paramName);
            }
        }
        ...
        internal static bool GetBooleanValue(NameValueCollection config, string valueName, bool defaultValue)
        {
            ...
            if (!bool.TryParse(str, out flag))
                throw new ProviderException(ResourceStringLoader.GetResourceString(
                    "Value_must_be_boolean", new object[] { valueName }));
        ...
        internal static int GetIntValue(NameValueCollection config, string valueName, int defaultValue, 
            bool zeroAllowed, int maxValueAllowed)
        {
            int num;
            ...
            if (!int.TryParse(s, out num))
            {
                if (zeroAllowed)
                    throw new ProviderException(ResourceStringLoader.GetResourceString(
                        "Value_must_be_non_negative_integer", new object[] { valueName }));

                throw new ProviderException(ResourceStringLoader.GetResourceString(
                    "Value_must_be_positive_integer", new object[] { valueName }));
            }
            ...
            if ((maxValueAllowed > 0) && (num > maxValueAllowed))
                throw new ProviderException(ResourceStringLoader.GetResourceString(
                    "Value_too_big", new object[] { valueName, maxValueAllowed.ToString(CultureInfo.InvariantCulture) }));
            ...
            bool checkForCommas, int maxSize)
        {
            ...
            param = param.Trim();
            return (((!checkIfEmpty || (param.Length >= 1)) && ((maxSize <= 0) || 
                (param.Length <= maxSize))) && (!checkForCommas || !param.Contains(",")));
        }
